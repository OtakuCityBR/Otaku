/**
 * SISTEMA FRIEREN GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Frieren
 */
class FrierenGallery {
    constructor() {
        ...
        this.images = [
            ...
            "https://i.ibb.co/gZWVbQqN/f-5.jpg",
            "https://i.ibb.co/LhDNVq4g/f-6.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando FrierenGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando FrierenGallery...');
        ...
        console.log('FrierenGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Frieren - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🧝‍♀️ Olá! Quero esta placa do Frieren (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO FRIEREN
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('frieren.html')) {
        console.log('Página de gallery Frieren detectada - inicializando...');
        const frierenGallery = new FrierenGallery();
    }
});
