/**
 * SISTEMA POKEMON GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Pokémon
 */
class PokemonGallery {
    constructor() {
        // Lista de imagens do Pokémon
        this.images = [
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando PokemonGallery');
            return;
        }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Pokémon - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `⚡ Olá! Quero esta placa do Pokémon (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO POKÉMON
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('pokemon.html')) {
        console.log('Página de gallery Pokémon detectada - inicializando...');
