/**
 * SISTEMA INUYASHA GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Inuyasha
 */
class InuyashaGallery {
    constructor() {
        // Lista de imagens do Inuyasha
        this.images = [
            "https://i.ibb.co/fzXsQS3p/i-1.jpg",
            "https://i.ibb.co/0Pdz3Pn/i-2.jpg",
            "https://i.ibb.co/8gYSWwg5/i-3.jpg",
            "https://i.ibb.co/7xNpLXv1/i-4.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando InuyashaGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando InuyashaGallery...');
        ...
        console.log('InuyashaGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Inuyasha - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🐕 Olá! Quero esta placa do Inuyasha (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO INUYASHA
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('inuyasha.html')) {
        console.log('Página de gallery Inuyasha detectada - inicializando...');
        const inuyashaGallery = new InuyashaGallery();
    }
});
