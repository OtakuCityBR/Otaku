/**
 * SISTEMA OVERLORD GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Overlord
 */
class OverlordGallery {
    constructor() {
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando OverlordGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando OverlordGallery...');
        ...
        console.log('OverlordGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Overlord - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `💀 Olá! Quero esta placa do Overlord (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO OVERLORD
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('overlord.html')) {
        console.log('Página de gallery Overlord detectada - inicializando...');
        const overlordGallery = new OverlordGallery();
    }
});
