/**
 * SISTEMA VOCALOID GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o Vocaloid
 */
class VocaloidGallery {
    constructor() {
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando VocaloidGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando VocaloidGallery...');
        ...
        console.log('VocaloidGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Vocaloid - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🎵 Olá! Quero esta placa do Vocaloid (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO VOCALOID
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('vocaloid.html')) {
        console.log('Página de gallery Vocaloid detectada - inicializando...');
        const vocaloidGallery = new VocaloidGallery();
    }
});
