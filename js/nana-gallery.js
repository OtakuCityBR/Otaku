/**
 * SISTEMA NANA GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Nana
 */
class NanaGallery {
    constructor() {
        // Lista de imagens do Nana
        this.images = [
            "https://i.ibb.co/twYQktK3/n-1.jpg",
            "https://i.ibb.co/HTwHrdV4/n-2.jpg",
            "https://i.ibb.co/4RmWqQTY/n-3.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando NanaGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando NanaGallery...');
        ...
        console.log('NanaGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Nana - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🎸 Olá! Quero esta placa do Nana (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO NANA
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('nana.html')) {
        console.log('Página de gallery Nana detectada - inicializando...');
        const nanaGallery = new NanaGallery();
    }
});
