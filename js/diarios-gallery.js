/**
 * SISTEMA DIÁRIOS GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Diários de uma Apotecária
 */
class DiariosGallery {
    constructor() {
        // Lista de imagens dos Diários de uma Apotecária
        this.images = [
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando DiariosGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando DiariosGallery...');
        ...
        console.log('DiariosGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Diários de uma Apotecária - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🧪 Olá! Quero esta placa dos Diários de uma Apotecária (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DOS DIÁRIOS
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('diarios.html')) {
        console.log('Página de gallery Diários detectada - inicializando...');
        const diariosGallery = new DiariosGallery();
    }
});
