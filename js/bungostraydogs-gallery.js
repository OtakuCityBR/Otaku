/**
 * SISTEMA BUNGO STRAY DOGS GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Bungo Stray Dogs
 */
class BungoStrayDogsGallery {
    constructor() {
        // Lista de imagens do Bungo Stray Dogs
        this.images = [
            "https://i.ibb.co/nNNMw3BR/bsd-1.jpg",
            "https://i.ibb.co/kg9KtRmz/bsd-2.jpg",
            "https://i.ibb.co/mChVYRkx/bsd-3.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando BungoStrayDogsGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando BungoStrayDogsGallery...');
        ...
        console.log('BungoStrayDogsGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Bungo Stray Dogs - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `📚 Olá! Quero esta placa do Bungo Stray Dogs (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO BUNGO STRAY DOGS
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('bungostraydogs.html')) {
        console.log('Página de gallery Bungo Stray Dogs detectada - inicializando...');
        const bungostraydogsGallery = new BungoStrayDogsGallery();
    }
});
