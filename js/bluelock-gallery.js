/**
 * SISTEMA BLUE LOCK GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Blue Lock
 */
class BlueLockGallery {
    constructor() {
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando BlueLockGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando BlueLockGallery...');
        ...
        console.log('BlueLockGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Blue Lock - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `⚽ Olá! Quero esta placa do Blue Lock (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO BLUE LOCK
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('bluelock.html')) {
        console.log('Página de gallery Blue Lock detectada - inicializando...');
        const blueLockGallery = new BlueLockGallery();
    }
});
