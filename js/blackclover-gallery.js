/**
 * SISTEMA BLACK CLOVER GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Black Clover
 */
class BlackCloverGallery {
    constructor() {
        ...
        this.images = [
            ...
            "https://i.ibb.co/C5B2Kr25/B-4.jpg",
            "https://i.ibb.co/JRPsRxRg/B-5.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando BlackCloverGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando BlackCloverGallery...');
        ...
        console.log('BlackCloverGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Black Clover - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🍀 Olá! Quero esta placa do Black Clover (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO BLACK CLOVER
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('blackclover.html')) {
        console.log('Página de gallery Black Clover detectada - inicializando...');
        const blackCloverGallery = new BlackCloverGallery();
    }
});
