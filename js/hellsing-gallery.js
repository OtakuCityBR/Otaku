/**
 * SISTEMA HELLSING GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Hellsing
 */
class HellsingGallery {
    constructor() {
        // Lista de imagens do Hellsing
        this.images = [
            "https://i.ibb.co/3m0ZwFWq/hs-1.jpg",
            "https://i.ibb.co/QzBgd0B/hs-2.jpg",
            "https://i.ibb.co/W4GwJTd6/hs-3.jpg",
            "https://i.ibb.co/3mf6SCH7/hs-4.jpg",
            "https://i.ibb.co/trWZSTC/hs-5.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando HellsingGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando HellsingGallery...');
        ...
        console.log('HellsingGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Hellsing - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🧛 Olá! Quero esta placa do Hellsing (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO HELLSING
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('hellsing.html')) {
        console.log('Página de gallery Hellsing detectada - inicializando...');
        const hellsingGallery = new HellsingGallery();
    }
});
