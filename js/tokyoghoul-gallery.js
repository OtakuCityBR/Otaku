/**
 * SISTEMA TOKYO GHOUL GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Tokyo Ghoul
 */
class TokyoGhoulGallery {
    constructor() {
        // Lista de imagens do Tokyo Ghoul
        this.images = [
            "https://i.ibb.co/mFFCTgnZ/tg-1.jpg",
            "https://i.ibb.co/xt23wwg4/tg-2.jpg",
            "https://i.ibb.co/ksw6VgGx/tg-3.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando TokyoGhoulGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando TokyoGhoulGallery...');
        ...
        console.log('TokyoGhoulGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Tokyo Ghoul - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `👁️ Olá! Quero esta placa do Tokyo Ghoul (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO TOKYO GHOUL
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('tokyoghoul.html')) {
        console.log('Página de gallery Tokyo Ghoul detectada - inicializando...');
        const tokyoGhoulGallery = new TokyoGhoulGallery();
    }
});
