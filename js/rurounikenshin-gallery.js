/**
 * SISTEMA RUROUNI KENSHIN GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Rurouni Kenshin
 */
class RurouniKenshinGallery {
    constructor() {
        // Lista de imagens do Rurouni Kenshin
        this.images = [
            "https://i.ibb.co/tMtLyXvF/rk-1.jpg",
            "https://i.ibb.co/tTdt4Wc1/rk-2.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando RurouniKenshinGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando RurouniKenshinGallery...');
        ...
        console.log('RurouniKenshinGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Rurouni Kenshin - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `⚔️ Olá! Quero esta placa do Rurouni Kenshin (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO RUROUNI KENSHIN
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('rurounikenshin.html')) {
        console.log('Página de gallery Rurouni Kenshin detectada - inicializando...');
        const rurouniKenshinGallery = new RurouniKenshinGallery();
    }
});
